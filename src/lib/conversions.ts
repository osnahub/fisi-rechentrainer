 ...
 */
export function parseDecimalInput(input: string): ParseResult<number> {
 ...
 */
export function parseBinaryInput(input: string): ParseResult<{ clean: string; value: number }> {
 ...
 * Handles leading/trailing whitespace correctly even before '0x'.
 */
export function parseHexInput(input: string): ParseResult<{ clean: string; value: number }> {
  const trimmed = input.trim();
  if (!trimmed) {
    return { ok: false, error: "Eingabe darf nicht leer sein." };
  }
  const clean = trimmed.replace(/^0x/i, "").toUpperCase();
  if (!clean) {
    return { ok: false, error: "Eingabe darf nicht leer sein." };
  }
 ...
 * Throws RangeError if dec exceeds either 32-bit unsigned range OR bitCount capacity.
 */
export function decToBin(dec: number, bitCount = 8): string {
  ...
  const maxCapacity = bitCount === 32 ? MAX_UINT32 : Math.pow(2, bitCount) - 1;
  if (dec > maxCapacity) {
    throw new RangeError(
      `decToBin: Wert ${dec} passt nicht in ${bitCount} Bit (maximaler Wert: ${maxCapacity}).`
    );
  }
 ...
 */
export function binToDec(binStr: string): number {
 ...
 */
export function decToHex(dec: number, pad = 2): string {
  if (!Number.isInteger(dec) || dec < MIN_UINT32 || dec > MAX_UINT32) {
    throw new RangeError(
 ...
 */
export function hexToDec(hexStr: string): number {
 ...
 */
export function hexToBin(hexStr: string): string {
  const parsed = parseHexInput(hexStr);
  if (!parsed.ok) {
    throw new Error(parsed.error);
  }
...
export function formatNibbles(binStr: string): string {
...
export function getPolynomialExpansion(binStr: string): PolynomialTerm[] {
  const clean = binStr.replace(/\s+/g, "");
  if (!clean || !/^[01]+$/.test(clean)) {
    throw new Error("getPolynomialExpansion: Ungültiges Binärmuster. Nur '0' und '1' erlaubt.");
  }
...
export function getStellenwertSteps(decimalNumber: number, bitCount = 8): StellenwertStep[] {
  ...
  if (!Number.isInteger(bitCount) || bitCount < 1 || bitCount > 32) {
    throw new RangeError("getStellenwertSteps: bitCount muss zwischen 1 und 32 liegen.");
  }
...
export function getDivisionSteps(decimalNumber: number): DivisionStep[] {
  if (!Number.isInteger(decimalNumber) || decimalNumber < MIN_UINT32 || decimalNumber > MAX_UINT32) {
    throw new RangeError(
...
export function getHexDivisionSteps(decimalNumber: number): HexDivisionStep[] {
  if (!Number.isInteger(decimalNumber) || decimalNumber < MIN_UINT32 || decimalNumber > MAX_UINT32) {
    throw new RangeError(
...
export function getSubnetExample(cidrSuffix: number): SubnetExampleProfile {
  if (!Number.isInteger(cidrSuffix) || cidrSuffix < 24 || cidrSuffix > 32) {
    throw new RangeError(
      `getSubnetExample: cidrSuffix muss eine Ganzzahl zwischen 24 und 32 sein (erhalten: ${cidrSuffix}).`
    );
  }

  const cidr = cidrSuffix;
